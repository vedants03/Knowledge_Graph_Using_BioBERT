...
# Initialize the global knowledge graph
...
def add_to_knowledge_graph(entities, relationships, knowledge_graph):
    for entity in entities:
        knowledge_graph.add_node(entity["id"], label=entity["label"], text=entity["text"])

    for relationship in relationships:
        knowledge_graph.add_edge(relationship["subject"], relationship["object"], label=relationship["label"])
...
def display_knowledge_graph(long_relation_df: pd.DataFrame, num_col: int = 2,
    ...
    for i, d in enumerate(drug_ids):
        ...
        # add_to_knowledge_graph(entities, relationships, global_knowledge_graph)
...
def clean_string(input_string):
    ...
    # Remove a full stop from the end, if present
    if cleaned_string.endswith('.'):
        cleaned_string = cleaned_string[:-1]

    return cleaned_string

def find_specific_word(question):
    # Define a list of specific words to check for
    question = clean_string(question)
    specific_words = [
        'frequency',
        'strength',
        'dosage',
        'ade', 'sideeffects',  # These words are combined for matching
        'reason', 'reasons', 'suffering',    # These words are combined for matching
        'dosage',
        'route', 'routes'
    ]

    # Create a regular expression pattern to match any of the specific words
    pattern = r'\b(?:' + '|'.join(map(re.escape, specific_words)) + r')\b'

    # Use the re.search function to find the first specific word in the question
    match = re.search(pattern, question, re.IGNORECASE)
    
    if match:
        return match.group(0)  # Return the specific word found
    else:
        return None  # Return None if no specific word is found